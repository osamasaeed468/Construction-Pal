...
import 'wagers_main.dart';

class WagersProfile extends StatefulWidget {
  @override
  _WagersProfileState createState() => _WagersProfileState();
}

class _WagersProfileState extends State<WagersProfile> {
  dynamic data;
  String type = "";
  ...
  void initState() {
    super.initState();
    getData();
    user=Helper.fetchDailyWagerValue(FirebaseAuth.instance.currentUser!.uid);
  }
  File? _pickedImage;
        ...
        builder: (BuildContext context) {
          return Scaffold(
            ...
            floatingActionButton: FloatingActionButton.extended(
              ...
              icon: Icon(Icons.add),
              label: Text('Add Service',style: const TextStyle(
                  fontWeight: FontWeight.bold,
                  fontFamily: 'MetroPolis'
              ),),
              onPressed: () {
                addService(context, FirebaseAuth.instance.currentUser!.uid);
              },
            ),
          );
        });
  ...
  Future<void> addService(BuildContext context, String cUid) async {
    ...
    var var1=await FirebaseFirestore.instance.collection("Wagers").doc(FirebaseAuth.instance.currentUser!.uid).get();
    String types=var1.data()!['type'].toString();
    print(types);
    if(types.contains("Plumber")){
      expertise.add("Plumber");
    }
    if(types.contains("Carpenter")){
      expertise.add("Carpenter");
    }
    if(types.contains("Welder")){
      expertise.add("Welder");
    }
    if(types.contains("AC Repair")){
      expertise.add("AC Repair");
    }
    if(types.contains("Electrician")){
      expertise.add("Electrician");
    }
    int i=0;
    for(i;i<expertise.length;i++){
      print('Expertise:     '+expertise[i]);
    }
    return showDialog(
        context: context,
        // barrierDismissible: false,
                                      ...
                                      child: DropdownButton<String>(
                                        ...
                                        onChanged: (String? newValue) {
                                          setState(() async {
                                            ...
                                            querySnapshot =
                                            await FirebaseFirestore.instance.collection("WagerServices").where("cat",isEqualTo: newValue).get();
                  ...
                  child: Container(
                      ...
                      child: Padding(
                          ...
                          child: new Column(children: <Widget>[
                            ...
                            TextField(
                              decoration: InputDecoration(
                                  labelText: "Expertise i.e Plumber, Welder",
                                  border: OutlineInputBorder(
